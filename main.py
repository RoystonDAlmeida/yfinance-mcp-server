...
# Create an MCP server with a custom name and explicit stdio transport
...
def get_stock_price(symbol: str) -> float:
	...
	try:
		...
		if not data.empty:
			...
			# As a fallback, try using the regular market price using ticker.info
			info = ticker.info
		...
		# Return -1.0 to indicate an error occurred while fetching
		return -1.0
...
def get_stock_history(symbol: str, period: str = "1mo") -> str:
	"""
	Retrieve historical data for a stock given a ticker symbol
	Returns the historical data as a CSV formatted string
	
	Parameters:
		symbol: The stock ticker symbol
		period: The period over which to retrieve historical data
	"""
	...
	try:
		...
		return f"Error fetchding data: {str(e)}"
